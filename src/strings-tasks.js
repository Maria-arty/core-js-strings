...
function sumOfCodes(str) {
  if (typeof str === 'string') {
    ...
    let result = 0;
    while (!theChar.done) {
      result += theChar.value.charCodeAt(0);
...
function countVowels(str) {
  ...
  if (typeof str === 'string') {
    ...
    let theChar = iterator.next();
    while (!theChar.done) {
      if (vowels.includes(theChar.value)) {
        result += 1;
      }
      theChar = iterator.next();
    }
...
function isPalindrome(str) {
  const ourStr = str.toLowerCase().replace(/[^a-z0-9]/g, '');
  const reversedStr = ourStr.split('').reverse().join('');
  return ourStr === reversedStr;
}
...
function encodeToRot13(str) {
  ...
  arr.forEach((char) => {
    if (char >= 'A' && char <= 'Z') {
      newChar = String.fromCharCode(((char.charCodeAt(0) - 65 + 13) % 26) + 65);
    } else if (char >= 'a' && char <= 'z') {
      newChar = String.fromCharCode(((char.charCodeAt(0) - 97 + 13) % 26) + 97);
    } else {
      newChar = char;
    }
    result += newChar;
